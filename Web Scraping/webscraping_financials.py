...
entities = ['financials', 'balance-sheet', 'cash-flow']
for ticker in tickers:
    #scraping income statement
    for entity in entities:
        
        url = "https://finance.yahoo.com/quote/{}/{}?p={}".format(ticker, entity, ticker)
        entity_dict = {}
        table_title = {}
        ...
        for t in tabl:
            ...
            for row in rows:
                entity_dict[row.get_text(separator="|").split("|")[0]] = row.get_text(separator="|").split("|")[1:]
    
        temp = pd.DataFrame(entity_dict).T
        temp.columns = table_title["Breakdown"]
        if entity == 'financials':
            income_statatement_dict[ticker] = temp
        elif entity == 'balance-sheet':
            balance_sheet_dict[ticker] = temp
        elif entity == 'cash-flow':
